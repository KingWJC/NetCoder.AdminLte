...
namespace NetCode.Business.Base_SysManage
{
    public class RapidDevelopmentBusiness : BaseBusiness<Base_DatabaseLink>, IRapidDevelopmentBusiness, IDependency
    {
        ...
        public void BuildCode(string linkId, string areaName, string tables, string buildType)
        {
            ...
            {
                var tableFieldInfo = _dbHelper.GetDbFieldInfo(aTable);
                //实体层
        ...
        private void BuildEntity(List<DbFieldInfo> tableInfo, string areaName, string tableName)
        {
            string rootPath = AppDomain.CurrentDomain.BaseDirectory;
            string entityPath = rootPath.Replace("NetCode.Web", "NetCode.Entity") + areaName;
            string filePath = $@"{entityPath}\{tableName}.cs";
            string nameSpace = $@"NetCode.Entity.{areaName}";
        ...
        private void BuildIBusiness(string areaName, string entityName)
        {
            ...
            string code =
$@"using NetCode.Entity.{areaName};
using NetCode.Util;
using System.Collections.Generic;

namespace NetCode.Business.{areaName}
{{
    ...
    {{
        ...
        AjaxResult DeleteData(List<string> ids);
            ...
            string rootPath = AppDomain.CurrentDomain.BaseDirectory.Replace("NetCode.Web", "NetCode.Business"); ;
...
{{
    ...
    {{
        ...
        private void BuildController(string areaName, string entityName)
        {
            ...
            string code =
$@"using NetCode.Business.{areaName};
using NetCode.Entity.{areaName};
using NetCode.Util;
using System.Web.Mvc;

namespace NetCode.Web.Areas.{areaName}.Controllers
{{
    public class {entityName}Controller : MvcBaseController
    {{
        #region DI
        ...
        /// 生成视图
        /// </summary>
        /// <param name="fieldInfoList">表字段信息</param>
        /// <param name="areaName">区域名</param>
        /// <param name="entityName">实体名</param>
        private void BuildView(List<DbFieldInfo> fieldInfoList, string areaName, string entityName)
        {
            //生成Index页面
            ...
            var formHeight = fieldInfoList.Where(x => x.Name != "Id").Count() * 2;
            if (formHeight > 8)
                formHeight = 8;
            fieldInfoList.Where(x => x.Name != "Id").ForEach((aField, index) =>
            {
                //搜索的下拉选项
...
namespace NetCode.Web.Areas.{areaName}
{{
    ...
    {{
        ...
        {{
            ...
            if (!File.Exists(areaRegistrationPath))
