...
namespace NetCode.Util
{
    public static partial class Extension
    {
        ...
        public static List<T> ToList<T>(this DataTable dt)
        {
            ...
            {
                //创建泛型对象
                //对于值类型和具有默认构造函数的引用类型，使用Activator.CreateInstance(Type)
                //检查没有默认构造函数的类，使用FormatterServices.GetUninitializedObject(Tyep)            此方法不会调用默认构造函数，所有字段都保持默认值。
        ...
        public static List<T> EmitToList<T>(this DataTable dt)
        {
            ...
            {
                //创建泛型对象
                T _t = (T)objBuilder();//是否也可以使用assemly.createinstance
                //获取对象所有属性
