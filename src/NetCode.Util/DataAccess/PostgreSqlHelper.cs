...
where c.relname = @table_name and a.attrelid = c.oid and a.attnum > 0;";
            return GetListBySql<DbFieldInfo>(sql, new List<DbParameter> { new NpgsqlParameter("@table_name", tableName) });
        }
        ...
        public override void SaveEntityToFile(List<DbFieldInfo> infos, string tableName, string tableDescription, string filePath, string nameSpace, string schemaName = "public")
        {
