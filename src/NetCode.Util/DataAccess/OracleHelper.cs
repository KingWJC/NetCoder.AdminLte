...
namespace NetCode.Util
{
    ...
    public class OracleHelper : DbHelper
    {
        ...
        {
            DbProviderFactory dbProviderFactory = DbObjectHelper.GetDbProviderFactory(_dbType);
...
where owner =:schemaName";
        ...
        /// <returns></returns>
        public override List<DbFieldInfo> GetDbFieldInfo(string tableName)
        {
            string sql = @"select
 ...
 where a.table_name = b.table_name      
       ...
       order by a.TABLE_NAME, a.COLUMN_ID";
            return GetListBySql<DbFieldInfo>(sql, new List<DbParameter> { new OracleParameter("table_name", tableName) });
        }
        ...
        public override void SaveEntityToFile(List<DbFieldInfo> infos, string tableName, string tableDescription, string filePath, string nameSpace, string schemaName = null)
        {  
